...
def remodel(rosetta_path, pdb_path, blueprint_path, outfolder='./rosetta_out'):
    ...
    command = "{} -database {} -s {} -remodel:blueprint {} -run:chain A -remodel:dr_cycles 3 -nstruct 1 -overwrite -ex1 -ex2 -ignore_zero_occupancy false -ignore_unrecognized_res true  -out:path:pdb {} -out:path:score {}".format(
        '\''+binary_path+'\'',
        '\''+database_path+'\'',
        '\''+pdb_path+'\'',
        '\''+blueprint_path+'\'',
